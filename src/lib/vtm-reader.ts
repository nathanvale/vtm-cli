...
import { VTM, Task, TaskContext } from './types';
...
export class VTMReader {
  ...
  async getReadyTasks(): Promise<Task[]> {
    ...
    const completed = new Set(
      vtm.tasks.filter(t => t.status === 'completed').map(t => t.id)
    );
  ...
  async getTaskWithContext(id: string): Promise<TaskContext> {
    ...
    const blockedTasks = vtm.tasks.filter(t => 
      t.dependencies.includes(id) && t.status === 'pending'
    );
  ...
  async getStatsByADR(): Promise<Record<string, { total: number; completed: number }>> {
    ...
    vtm.tasks.forEach(task => {
      ...
      stats[task.adr_source].total++;
      if (task.status === 'completed') {
        stats[task.adr_source].completed++;
      }
    });
